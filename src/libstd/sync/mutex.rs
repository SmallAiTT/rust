...
impl StaticMutex {
    ...
    pub fn trylock<'a>(&'a self) -> Option<Guard<'a>> {
        ...
        if unsafe { this.lock.trylock() } {
            this.held.store(true, atomics::Relaxed); // see below
            Some(Guard{ lock: this })
        } else {
            None
        }
    }
    ...
    pub fn lock<'a>(&'a self) -> Guard<'a> {
        ...
        if t.can_block() {
            ...
            t.deschedule(1, |task| unsafe {
                ...
                while !this.held.load(atomics::Relaxed) {
                    if this.lock.trylock() {
                        this.held.store(true, atomics::Relaxed);
                        stolen = true;
                        break
                    } else {
                        Thread::yield_now();
            ...
            assert!(this.held.load(atomics::Relaxed));
        }
    ...
    fn unlock(&self) {
        ...
        assert!(this.held.load(atomics::Relaxed));
        this.held.store(false, atomics::Relaxed);
    ...
    pub unsafe fn destroy(&self) {
        cast::transmute_mut(self).lock.destroy()
    }
}
...
impl Mutex {
    ...
    pub fn trylock<'a>(&'a mut self) -> Option<Guard<'a>> { self.lock.trylock()
    }
